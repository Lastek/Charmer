...
int main() {
    ...
    // --- REPLACE THIS PATH WITH THE ONE FROM YOUR DIRECTSHOW OUTPUT ---
    std::wstring devicePath = L"\\\\?\\usb#vid_174f&pid_11a8&mi_00#7&20e0e622&1&0000#{65e8773d-8f56-11d0-a3b9-00a0c9223196}\\global"; // example

    std::wcout << L"Trying device path: " << devicePath << std::endl;

    IMFMediaSource* pSource = nullptr;
    hr = CreateMediaSourceFromPath(devicePath, &pSource);
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            for (DWORD streamIndex = 0; ; streamIndex++) {
                ...
                while (true) {
                    ...
                    if (hr == MF_E_INVALIDSTREAMNUMBER) goto done_streams;
                    if (SUCCEEDED(hr)) {
        ...
        if (hr == 0xC00D36E4L) std::cout << "    Device in use (disable Windows Hello temporarily).\n";
    }
